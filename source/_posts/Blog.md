...
title: 个人博客搭建
author: Luckey
photos: https://api.flymc.cc/hotlink-ok/cover/61351818_p0.jpg
summary: 前前后后大概花了2周的时间，目前个人博客已经完善的差不多了，现在写个文章做个阶段总结，后续如果有更新的地方，会及时补充。本博客基于Hexo框架，采用hexo-theme-matery主题，并在此基础上做了改进。
tags:
  - Hexo
  - Github
  - 博客
...
---
<div class="note danger">本文写的时间较早，请核查文章准确性！</div>
...
前面`git`和`nodejs`安装好后，就可以安装`hexo`了，你可以先创建一个文件夹`MyBlog`，用来存放自己的博客文件，然后`cd`到这个文件夹下（或者在这个文件夹下直接右键`git  `打开）。
...
在`git  `输入`ssh -T git@github.com`，如果如下图所示，出现你的用户名，那就成功了。
...
首先在博客根目录下右键打开`git   here`，安装一个扩展`npm i hexo-deployer-git`。
